...
import { BarChart3, Users, FolderOpen, TrendingUp, CheckCircle2, Clock, Search, Calendar, Target, Building } from "lucide-react";
...
const ProgressMonitoring = () => {
  ...
  const filteredUsers = users.filter((user) =>
    ...
    user.project.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  placeholder={`Buscar ${activeTab === "projects" ? "proyectos..." : "usuarios o proyectos..."}`}
                  value={searchTerm}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              project.status === 'finalizada' ? 'bg-green-100 text-green-800' :
              ...
              {filteredUsers.length === 0 ? (
                ...
                <div className="space-y-4">
                  {filteredUsers.map((user) => (
                    <div key={`${user.id}-${user.project.id}`} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-medium text-gray-700">
                              Proyecto: {user.project.name}
                            </span>
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              user.project.status === 'finalizada' ? 'bg-green-100 text-green-800' :
                              user.project.status === 'En Progreso' ? 'bg-blue-100 text-blue-800' :
                              user.project.status === 'Pendiente' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-gray-100 text-gray-800'
                            }`}>
                              {user.project.status}
                            </span>
                          ...
                          <p className="text-gray-600 text-sm">
                            {user.completedTasks} de {user.totalTasks} tareas completadas en este proyecto
                          </p>
                        </div>
                        <div className="text-right">
                          <span className={`text-xl font-bold ${
