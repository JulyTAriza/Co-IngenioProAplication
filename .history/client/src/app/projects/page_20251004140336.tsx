...
const ProjectsPage = () => {
  ...
  // Query para detalles del proyecto - CORREGIDO: usar directamente la respuesta
  const { 
    data: proyectoDetalle, // Ya es ProyectoResumen directamente por el transformResponse
  ...
  const handleCreateProyecto = async (proyectoData: any) => {
    try {
      ...
      console.error("Error al crear/editar proyecto:", {
        message: err.message,
        data: err.data,
        status: err.status
      });
      alert(`Error: ${err.data?.message || "No se pudo procesar el proyecto. Revisa los campos."}`);
    }
  };
  ...
  useEffect(() => {
    if (proyectoDetalle && proyectoIdParaDetalle && !loadingDetalle) {
      ...
      // proyectoDetalle ya es el objeto ProyectoResumen directamente
  ...
  return (
    <div className="mx-auto pb-5 w-full">
            ...
            {filteredProyectos.length === 0 ? (
              ...
              filteredProyectos.map((proyecto: ProyectoResumen) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">{proyecto.nombre_cliente}</td>
                  <td className="py-3 px-4">{proyecto.nombre_ciudad}</td>
      ...
      {/* CONFIRM DIALOG - CORREGIDO: eliminada la prop isLoading */}
      <ConfirmDialog
        isOpen={confirmOpen}
        title="Eliminar proyecto"
