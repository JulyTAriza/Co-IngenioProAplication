...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (VALIDANDO PROYECTO Y USUARIO)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        cursor.execute("SELECT id_proyecto FROM etapas_proyecto WHERE id_etapa = ?", (id_etapa,))
        ...
        id_proyecto_etapa = etapa[0]
        print(f"✅ [CREAR-ACTIVIDAD] Etapa pertenece al proyecto: {id_proyecto_etapa}")
        
        # 2️⃣ VALIDACIÓN COMPLETA DEL ID_PERSONAL
        id_personal = data.get("id_personal")
        print(f"🔍 [CREAR-ACTIVIDAD] ID Personal recibido: {id_personal}")
        
        if id_personal:
            # VERIFICAR QUE EL PERSONAL PERTENEZCA AL MISMO PROYECTO DE LA ETAPA
            cursor.execute("""
                SELECT pp.id_personal_proyecto, pp.id_proyecto, pp.id_usuario, u.username 
                FROM personal_proyecto pp
                LEFT JOIN Usuarios u ON pp.id_usuario = u.id
                WHERE pp.id_personal_proyecto = ?
            """, (id_personal,))
            ...
            if not personal_info:
                conn.close()
                return jsonify(success=False, message=f"El personal con ID {id_personal} no existe"), 400
            
            id_proyecto_personal = personal_info[1]
            id_usuario_personal = personal_info[2]
            username_personal = personal_info[3]
            
            print(f"🔍 [CREAR-ACTIVIDAD] Personal encontrado: Proyecto={id_proyecto_personal}, Usuario={username_personal} (ID:{id_usuario_personal})")
            
            # VALIDAR QUE PERTENEZCA AL MISMO PROYECTO
            if id_proyecto_personal != id_proyecto_etapa:
                conn.close()
                return jsonify(
                    success=False, 
                    message=f"El personal con ID {id_personal} pertenece al proyecto {id_proyecto_personal}, no al proyecto {id_proyecto_etapa} de esta etapa"
                ), 400
            
            # VALIDAR QUE TENGA USUARIO VÁLIDO
            if not id_usuario_personal:
                conn.close()
                return jsonify(
                    success=False, 
                    message=f"El personal con ID {id_personal} no tiene un usuario válido asignado"
                ), 400
            
            print(f"✅ [CREAR-ACTIVIDAD] Personal válido para este proyecto")
