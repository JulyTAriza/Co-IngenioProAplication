...
# ---------------------------
def enviar_correo_vencimiento(asunto, template, usuario_email, datos_tarea):
    """
    Función para enviar correos sobre vencimiento de tareas
    """
    try:
        ...
        html_body = render_template(f"emails/{template}.html", **datos_tarea)
...
# ---------------------------
# ENDPOINT PARA EJECUTAR VERIFICACIÓN MANUAL DE VENCIMIENTOS
# ---------------------------
@my_tasks_bp.route("/verificar-vencimientos", methods=["POST", "OPTIONS"])
@token_required
def verificar_vencimientos(current_user):
    """
    Endpoint para ejecutar manualmente la verificación de tareas próximas a vencer
    ...
    try:
        ...
        notificaciones_enviadas = verificar_tareas_proximas_a_vencer()
        
        return jsonify({
            "success": True,
            "message": f"Verificación completada. Se enviaron {notificaciones_enviadas} notificaciones de vencimiento."
        }), 200
        
    except Exception as e:
        print(f"❌ ERROR en verificar_vencimientos: {str(e)}")
...
# ---------------------------
# REABRIR TAREA (CON ACTUALIZACIÓN AUTOMÁTICA DE PROYECTO)
# ---------------------------
...
def reopen_task(current_user, task_id):
    ...
    Y actualiza el proyecto si estaba finalizado
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    ...
    try:
        ...
        task_name = task[1]
        task_user_id = task[2]
        project_status = task[3]
        project_id = task[4]
        project_name = task[5]
        
        print(f"📝 [REOPEN-TASK] Tarea encontrada: ID={task_id_db}, Nombre='{task_name}', Proyecto='{project_name}' (ID={project_id}), EstadoProyecto='{project_status}'")
        ...
        # Si el proyecto estaba finalizado, cambiar a en progreso
        project_reopened = False
        if project_status == 'finalizada':
            cursor.execute("""
                UPDATE proyectos
            ...
            print(f"🔄 [REOPEN-TASK] Proyecto {project_id} cambiado de 'finalizada' a 'en progreso'")
        ...
        if project_reopened:
            return jsonify(
                success=True, 
                message="Tarea reabierta correctamente. El proyecto ha sido reactivado.",
