...
# ---------------------------
# ACTUALIZAR ACTIVIDAD (CON MISMA LÓGICA)
# ---------------------------
...
def actualizar_actividad(current_user, id_actividad):
    ...
    try:
        ...
        # 1️⃣ OBTENER EL PROYECTO DE LA ACTIVIDAD
        cursor.execute("""
            SELECT a.id_proyecto, p.nombre as proyecto_nombre, p.estado as proyecto_estado
            FROM actividades_cronograma a
            INNER JOIN proyectos p ON a.id_proyecto = p.id_proyecto
            WHERE a.id_actividad = ? AND p.estado <> 'Eliminado'
        """, (id_actividad,))
        ...
        if not actividad_info:
            conn.close()
            return jsonify(success=False, message="Actividad no encontrada o proyecto eliminado"), 404
            
        id_proyecto = actividad_info[0]
        nombre_proyecto = actividad_info[1]
        
        # 2️⃣ CONVERTIR ID_USUARIO EN ID_PERSONAL_PROYECTO (si se proporciona)
        id_usuario_asignado = data.get("id_personal")
        id_personal_final = None
        
        if id_usuario_asignado:
            # Validar usuario y conversión (misma lógica que crear_actividad)
            ...
            """, (id_usuario_asignado, id_proyecto))
            ...
            if not personal_info:
                ...
                return jsonify(
                    success=False, 
                    message=f"El usuario no está asignado al proyecto {nombre_proyecto}"
                ), 400
            
            id_personal_final = personal_info[0]
        
        # 3️⃣ ACTUALIZAR LA ACTIVIDAD
        cursor.execute("""
        ...
        print(f"❌ ERROR actualizando actividad: {str(e)}")
