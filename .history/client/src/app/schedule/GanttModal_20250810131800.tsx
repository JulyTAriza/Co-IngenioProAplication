...
import { formatDateShort, startOfDay, genId, inputToDate, dateToInputValue } from "./helpers";
...
/**
 * Props:
 *  - project: Project (trabajar sobre copia local)
 *  - onClose
 *  - onSave
 */
type Props = {
...
export default function GanttModal({ project, onClose, onSave }: Props) {
  const [tasks, setTasks] = useState<TaskExtended[]>(() => project.tasks ? project.tasks.map(t => ({ ...t })) : []);
  ...
  // derived users
  const users = useMemo(() => Array.from(new Set<string>(["Sin asignar", ...tasks.map(t => t.responsible ?? "Sin asignar")])), [tasks]);
  ...
  useEffect(() => {
    // marca atrasadas al montar (demo)
  ...
  const saveForm = () => {
    ...
    if (form.end < form.start) { alert("La fecha fin no puede ser anterior a la fecha inicio."); return; }
    ...
    if (editingId) {
      setTasks(prev => prev.map(t => t.id === editingId ? {
        ...
        materialPercent: form.materialPercent ?? 0,
      } : t));
    } else {
  ...
  const onDateChange = (changed: any) => {
    setTasks(prev => prev.map(t => t.id === changed.id ? { ...t, start: new Date(changed.start), end: new Date(changed.end) } : t));
  };
  const onProgressChange = (changed: any) => {
    const newProgress = Number(changed.progress ?? 0);
    setTasks(prev => prev.map(t => t.id === changed.id ? { ...t, progress: newProgress, status: newProgress >= 100 ? "Completada" : t.status } : t));
  };
      ...
      <div className="bg-white w-full max-w-7xl rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div>
            ...
            <p className="text-sm text-gray-500">{project.start.toLocaleDateString()} → {project.end.toLocaleDateString()}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-1 rounded bg-blue-600 text-white" onClick={saveProjectAndClose}>Guardar</button>
          </div>
        </div>
        ...
        <div className="p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
          <TaskList
            ...
            setViewMode={(v) => setView(ViewMode[v as keyof typeof ViewMode])}
            viewMode={view.toString()}
          />

          {/* Gantt area spanning two columns */}
          <div className="lg:col-span-2 bg-white rounded p-3 overflow-auto">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm text-gray-600">Usuarios: {users.length}</div>
            </div>

            <GanttChart tasks={tasks} view={view} onDateChange={onDateChange} onProgressChange={onProgressChange} />
          </div>
        </div>
