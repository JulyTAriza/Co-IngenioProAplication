...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS CONSUMO ESTIMADO (CORREGIDO)
# ---------------------------
@inventory_bp.route("/stock-comparison", methods=["GET", "OPTIONS"])
@token_required
def get_stock_comparison(current_user):
    """
    Compara stock actual vs consumo estimado en proyectos ACTIVOS
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    ...
    try:
        ...
        # CONSULTA CORREGIDA - Filtrar SOLO proyectos ACTIVOS
        cursor.execute("""
            SELECT 
                m.id,
                m.nombre,
                ...
                COALESCE(SUM(
                    CASE 
                        WHEN p.estado = 'Activo' THEN me.cantidad 
                        ELSE 0 
                    END
                ), 0) as consumo_estimado,
                m.cantidad - COALESCE(SUM(
                    CASE 
                        WHEN p.estado = 'Activo' THEN me.cantidad 
                        ELSE 0 
                    END
            ...
            HAVING COALESCE(SUM(
                CASE 
                    ...
                    ELSE 0 
                END
            ), 0) > 0  -- Solo materiales con consumo estimado en proyectos activos
        ...
        print(f"✅ Comparación de {len(comparison)} materiales (solo con consumo estimado en proyectos activos)")
