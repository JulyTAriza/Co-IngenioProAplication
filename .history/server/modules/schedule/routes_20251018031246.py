...
def obtener_schedule_completo(current_user, id_proyecto):
    ...
    try:
        ...
        # 2️⃣ Por cada etapa, traer sus actividades CON INFORMACIÓN DE PERSONAL
        for e in etapas:
            cursor.execute("""
                SELECT a.id_actividad, a.nombre_actividad, a.fecha_inicio, a.fecha_fin, 
                ...
                "actividades": [
                    {
                        "id_actividad": a[0],
                        "nombre": a[1],
                        "fecha_inicio": a[2].strftime('%Y-%m-%d') if a[2] else None,
                        "fecha_fin": a[3].strftime('%Y-%m-%d') if a[3] else None,
                        "estado": a[4],
                        "id_personal": a[6],  # ← ENVIAR id_usuario (NO id_personal_proyecto)
                        "personal": a[7]      # ← username del personal
                    } for a in acts
                ]
            })
