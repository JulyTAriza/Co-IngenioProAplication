...
export default function ReportsPage() {
  ...
  // --- Reporte Etapas (requiere id_proyecto)
  const {
    data: repEtapas,
    isLoading: loadingEtapas,
    isError: errorEtapas,
  } = useGetReporteEtapasQuery(projectId ?? skipToken);

  // --- Reporte Avances (requiere id_etapa)
  const {
    data: repAvances,
    isLoading: loadingAvances,
    isError: errorAvances,
  } = useGetReporteAvancesQuery(etapaId ?? skipToken);

  // --- Reporte Materiales (no recibe parámetros)
  const {
    data: repMateriales,
  ...
  // --- Reporte Materiales por Etapa (requiere id_etapa)
  const {
    data: repMaterialesEtapa,
    isLoading: loadingMaterialesEtapa,
    isError: errorMaterialesEtapa,
  } = useGetReporteMaterialesPorEtapaQuery(etapaId ?? skipToken);

  // --- Reporte Personal por Proyecto (requiere id_proyecto)
  const {
    data: repPersonal,
    isLoading: loadingPersonal,
    isError: errorPersonal,
  } = useGetReportePersonalProyectoQuery(projectId ?? skipToken);

  // --- Reporte Estados de Personal (sin parámetros)
  const {
  ...
  // --- Reporte Clientes (sin parámetros)
  const {
    data: repClientes,
  ...
  // --- Reporte Resumen Proyecto (requiere id_proyecto)
  const {
    data: repResumen,
    isLoading: loadingResumen,
    isError: errorResumen,
  } = useGetReporteResumenProyectoQuery(projectId ?? skipToken);

  // --- Reporte Consumo de Materiales (requiere id_proyecto)
  const {
    data: repConsumoMateriales,
    isLoading: loadingConsumo,
    isError: errorConsumo,
  } = useGetReporteConsumoMaterialesQuery(projectId ?? skipToken);
