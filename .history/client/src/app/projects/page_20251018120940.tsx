...
const ProjectsPage = () => {
  ...
  // Query para detalles del proyecto - SIN CACHE
  const { 
  ...
  // Manejar la edición de proyecto - SIMPLIFICADO
  ...
  useEffect(() => {
    if (!proyectoIdParaDetalle || !lastRequestedId) return;
    ...
    if (proyectoDetalleResponse && !loadingDetalle) {
      ...
      const solicitadoId = lastRequestedId;
      ...
      if (respuestaId && respuestaId === solicitadoId) {
        ...
        // SOLUCIÓN AGREGIVA: Forzar recarga múltiple hasta que coincida
        console.log("🔄 [PAGE-DEBUG] Forzando recarga agresiva...");
        
        // Intentar varias veces con delays crecientes
        const attempts = [300, 600, 1000, 1500];
        attempts.forEach((delay, index) => {
          setTimeout(() => {
            console.log(`🔄 [PAGE-DEBUG] Recarga agresiva ${index + 1}/${attempts.length}`);
            refetchDetalle();
          }, delay);
        });
        
        // Si después de todos los intentos no funciona, mostrar error
        setTimeout(() => {
          if (proyectoDetalleResponse?.id_proyecto !== solicitadoId) {
            console.error("❌ [PAGE-DEBUG] No se pudo cargar el proyecto correcto después de múltiples intentos");
            alert("Error: El servidor está retornando datos incorrectos. Intente recargar la página o contacte al administrador.");
            setIsLoadingEdit(false);
            setProyectoIdParaDetalle(null);
            setLastRequestedId(null);
          }
  ...
  }, [proyectoDetalleResponse, loadingDetalle, errorDetalle, proyectoIdParaDetalle, lastRequestedId, refetchDetalle]);

  // Effect para forzar recarga inicial cuando se establece el ID
  useEffect(() => {
    if (proyectoIdParaDetalle && proyectoIdParaDetalle === lastRequestedId) {
      console.log("🔄 [PAGE-DEBUG] ID establecido, forzando recarga inicial:", proyectoIdParaDetalle);
