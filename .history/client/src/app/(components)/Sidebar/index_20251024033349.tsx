...
const SidebarLink = ({
  ...
  const isActive = 
    ...
    (pathname === "/dashboard-operario" && href === "/dashboard-operario") ||
    (pathname === "/dashboard" && href === "/dashboard");

  // Si es operario y trata de acceder a rutas no permitidas, prevenir comportamiento por defecto
  const handleClick = (e: React.MouseEvent) => {
    const allowedRoutes = ['/dashboard-operario', '/my-task', '/settings'];
    ...
    <Link href={href} onClick={handleClick}>
      <div
        ...
        hover:text-blue-600 hover:bg-blue-50 gap-3 transition-all duration-200 ${
          ...
          isOperario && !['/dashboard-operario', '/my-task', '/settings'].includes(href) 
...
const Sidebar = () => {
  ...
  const allLinks = [
    {
      href: isOperario ? "/dashboard-operario" : "/dashboard", // ← CAMBIO IMPORTANTE AQUÍ
      icon: Layout,
      label: "Dashboard",
