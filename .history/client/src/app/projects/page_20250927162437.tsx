...
const ProjectsPage = () => {
  ...
  const [selectedProyecto, setSelectedProyecto] = useState<any | null>(null); // ✅ Cambiado a any para datos completos
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [proyectoToDelete, setProyectoToDelete] = useState<number | null>(null);
  const [isLoadingEdit, setIsLoadingEdit] = useState(false); // ✅ Nuevo estado para loading
  ...
  // ✅ Hook para obtener detalles, pero controlado manualmente
  ...
  // ✅ FUNCIÓN CORREGIDA: Cargar datos completos antes de abrir el modal
  const handleEdit = async (proyecto: ProyectoResumen) => {
    try {
      ...
      // Activar la query para obtener datos completos
      setProyectoIdParaDetalle(proyecto.id_proyecto);
      
      // Esperar a que se carguen los datos
      // (React Query se encargará de hacer la petición)
  ...
  // ✅ Nuevo useEffect para manejar cuando se cargan los datos del proyecto
  React.useEffect(() => {
    if (proyectoDetalle && proyectoIdParaDetalle && !loadingDetalle) {
      console.log("✅ Datos del proyecto cargados:", proyectoDetalle);
      setSelectedProyecto(proyectoDetalle);
      setIsModalOpen(true);
      setIsLoadingEdit(false);
  ...
  }, [proyectoDetalle, loadingDetalle, errorDetalle, proyectoIdParaDetalle]);
              ...
              filteredProyectos.map((proyecto) => (
                <tr key={proyecto.id_proyecto} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 flex justify-center gap-3">
                    <button
                      className={`text-blue-500 hover:text-blue-700 ${isLoadingEdit ? 'opacity-50 cursor-not-allowed' : ''}`}
