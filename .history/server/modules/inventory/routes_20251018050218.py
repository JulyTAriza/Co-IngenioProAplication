...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS CONSUMO ESTIMADO (VERSIÓN SIMPLIFICADA)
# ---------------------------
@inventory_bp.route("/stock-comparison", methods=["GET", "OPTIONS"])
@token_required
def get_stock_comparison(current_user):
    """
    Compara stock actual vs consumo estimado en proyectos
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                m.nombre,
                m.cantidad as stock_actual,
                COALESCE(SUM(me.cantidad), 0) as consumo_estimado,
        ...
        for r in rows:
            ...
            # Solo incluir materiales que tienen consumo estimado
