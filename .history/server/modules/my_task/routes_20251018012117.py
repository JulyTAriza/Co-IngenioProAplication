...
# ---------------------------
# COMPLETAR TAREA (CON MEJOR MANEJO DE ERRORES)
# ---------------------------
...
def complete_task(current_user, task_id):
    ...
    try:
        ...
        # 2️⃣ Obtener todos los personal_proyecto del usuario
        cursor.execute("SELECT id_personal_proyecto FROM personal_proyecto WHERE id_usuario = ?", (id_usuario,))
        personal_rows = cursor.fetchall()
        personal_ids = [p[0] for p in personal_rows]
        
        print(f"✅ [COMPLETE-TASK] IDs de personal del usuario: {personal_ids}")
        
        # 3️⃣ Verificar que la tarea existe y está asignada a este usuario
        cursor.execute("""
            SELECT 
                a.id_actividad, 
                a.nombre_actividad, 
        ...
        if not task:
            conn.close()
            print(f"❌ [COMPLETE-TASK] Tarea {task_id} no encontrada")
        ...
        print(f"   - ID Personal asignado: {task[3]}")
        print(f"   - Usuario asignado: {task[4]}")
        print(f"   - Proyecto: '{task[5]}'")
        
        # 4️⃣ Verificar que la tarea está asignada a alguno de los personal_proyecto del usuario
        if task[3] not in personal_ids:
            conn.close()
            print(f"🚫 [COMPLETE-TASK] Permiso denegado:")
            print(f"   - Personal de la tarea: {task[3]}")
            print(f"   - Personales del usuario: {personal_ids}")
            print(f"   - La tarea NO está asignada al usuario actual")
            return jsonify(success=False, message="No tienes permiso para completar esta tarea"), 403
        
        # 5️⃣ Verificar que la tarea no esté ya completada
        ...
        # 6️⃣ Actualizar estado a "Completada"
