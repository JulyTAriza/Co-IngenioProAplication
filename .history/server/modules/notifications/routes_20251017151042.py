...
def load_email_template(template_name, **context):
    ...
    Estructura:
    server/
    ├── modules/notifications/routes.py (AQUÍ ESTAMOS)
    ├── templates/emails/
    └── app.py
    """
    try:
        # Subir 3 niveles: notifications -> modules -> server
        server_root = Path(__file__).parent.parent.parent
        template_path = server_root / 'templates' / 'emails' / f'{template_name}.html'
        ...
        raise Exception(f"Plantilla '{template_name}' no existe en {template_path}")
...
def check_critical_notifications():
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                # Determinar plantilla según Priority
