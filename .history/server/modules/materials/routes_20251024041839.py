...
def delete_material(current_user, material_id):
    """
    Eliminar material - SOLO ADMINISTRADORES
    """
    import pyodbc
    ...
    try:
        ...
        try:
            ...
            conn.rollback()
            conn.close()
            # Error 547 = conflicto con FOREIGN KEY
            if "FOREIGN KEY" in str(e) or "REFERENCE constraint" in str(e):
                return jsonify({
                    "success": False,
                    "message": f"No se puede eliminar el material '{material[0]}' porque está asociado a una o más etapas."
                }), 409
            else:
                raise  # Re-lanza si es otro tipo de error
