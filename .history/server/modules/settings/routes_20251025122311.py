...
def solicitar_codigo_contrasena(current_user, current_user_id, id_usuario):
    ...
    try:
        ...
        fecha_creacion = datetime.datetime.now()  # ← AGREGAR ESTO

        current_app.logger.info("🔐 [SOLICITAR CODIGO] Código generado: %s para %s", codigo, email)

        # Guardar código en la base de datos - CORREGIDO
        cursor.execute("""
            INSERT INTO CodigosVerificacion (id_usuario, codigo, intentos, fecha_expiracion, utilizado, fecha_creacion)
            VALUES (?, ?, 0, ?, 0, ?)
        """, (id_usuario, codigo, fecha_expiracion, fecha_creacion))  # ← AGREGAR fecha_creacion
        
        conn.commit()
        conn.close()

        # Enviar email con código de verificación usando la nueva función
        try:
            datos_correo = {
                "username": username,
                "codigo": codigo,
            ...
            enviar_correo_usuario(
                asunto="🔐 Código de Verificación - Cambio de Contraseña - Co-IngenioPro",
...
# ===================================
# 🔐 CAMBIAR CONTRASEÑA CON CÓDIGO (ACTUALIZADO CON CORREO AUTOMÁTICO)
# ===================================
...
def cambiar_contrasena(current_user, current_user_id, id_usuario):
    ...
    try:
        ...
        try:
            datos_correo = {
                "username": username,
                "fecha_actualizacion": datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
            }
            
            enviar_correo_usuario(
                asunto="✅ Contraseña actualizada - Co-IngenioPro",
                template="contrasena_actualizada",
                usuario_email=email,
                datos_usuario=datos_correo
            )
            current_app.logger.info("✅ [CAMBIAR CONTRASEÑA] Email de confirmación enviado a %s", email)

        except Exception as mail_error:
            current_app.logger.error("❌ [CAMBIAR CONTRASEÑA] Error enviando email: %s", mail_error)
            # No retornamos error aquí para no afectar el cambio de contraseña exitoso
