...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS CONSUMO ESTIMADO (VERSIÓN SIMPLE)
# ---------------------------
@inventory_bp.route("/stock-comparison", methods=["GET", "OPTIONS"])
@token_required
def get_stock_comparison(current_user):
    """
    Compara stock actual vs consumo estimado en TODOS los proyectos
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    ...
    try:
        ...
        print(f"✅ Comparación de {len(comparison)} materiales con consumo estimado")
