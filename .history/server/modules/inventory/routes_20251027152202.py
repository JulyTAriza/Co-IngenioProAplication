...
# ---------------------------
# DECORADORES: TOKEN JWT + ROLES (CON DEBUG)
# ---------------------------
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            print(f"🔐 DEBUG token_required - Token decodificado:")
...
# VERSIÓN DEBUG PARA ROLES
def role_required(roles_permitidos):
    def decorator(f):
        @wraps(f)
        def decorated(current_user, *args, **kwargs):
            print(f"🔐 DEBUG role_required - Usuario recibido: '{current_user}'")
            print(f"🔐 DEBUG role_required - Roles permitidos: {roles_permitidos}")
            ...
            try:
                ...
                cursor.execute("SELECT id, username, e_mail, rol FROM usuarios")
                todos_usuarios = cursor.fetchall()
                print(f"📋 DEBUG - Todos los usuarios en BD:")
                for usuario in todos_usuarios:
                    print(f"   ID: {usuario[0]}, Username: '{usuario[1]}', Email: '{usuario[2]}', Rol: '{usuario[3]}'")
                
                # Buscar el usuario actual por username
                print(f"🔍 DEBUG - Buscando usuario por username: '{current_user}'")
                cursor.execute("SELECT id, username, e_mail, rol FROM usuarios WHERE username = ?", (current_user,))
                usuario_por_username = cursor.fetchone()
                
                if usuario_por_username:
                    print(f"✅ DEBUG - Usuario encontrado por username: {usuario_por_username}")
                    rol_usuario = usuario_por_username[3]
                else:
                    # Si no encuentra por username, intentar por email
                    print(f"🔍 DEBUG - Buscando usuario por email: '{current_user}'")
                    cursor.execute("SELECT id, username, e_mail, rol FROM usuarios WHERE e_mail = ?", (current_user,))
                    usuario_por_email = cursor.fetchone()
                    
                    if usuario_por_email:
                        print(f"✅ DEBUG - Usuario encontrado por email: {usuario_por_email}")
                        rol_usuario = usuario_por_email[3]
                    else:
                        print(f"❌ DEBUG - Usuario NO encontrado ni por username ni por email")
                        conn.close()
                        return jsonify({
                            'success': False, 
                            'message': f'Usuario no encontrado en la base de datos: {current_user}'
                        }), 404
                
                conn.close()
                
                print(f"🎯 DEBUG - Rol del usuario: '{rol_usuario}'")
                print(f"🎯 DEBUG - Verificando si '{rol_usuario}' está en {roles_permitidos}")
                
                if rol_usuario not in roles_permitidos:
                    print(f"❌ DEBUG - Acceso DENEGADO: Rol '{rol_usuario}' no está en {roles_permitidos}")
                    return jsonify({
                        'success': False, 
...
def add_cors_headers(response):
    ...
    # Verificar si el origen está permitido (usar el que coincida o el primero por defecto)
...
# ---------------------------
# RESUMEN GENERAL DE INVENTARIO - SOLO ADMIN
# ---------------------------
...
def get_inventory_summary(current_user):
    """
    Retorna resumen general del inventario - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_inventory_summary para usuario: {current_user}")
        ...
        print(f"✅ Resumen de inventario calculado por {current_user}")
...
def get_stock_alerts(current_user):
    """
    Retorna materiales con stock bajo o agotado - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_stock_alerts para usuario: {current_user}")
        ...
        print(f"✅ {len(alerts)} alertas de stock consultadas por {current_user}")
...
# ---------------------------
# CONSUMO DE MATERIALES POR PROYECTO - SOLO ADMIN
# ---------------------------
...
def get_materials_consumption_by_project(current_user):
    """
    Retorna consumo ESTIMADO de materiales por proyecto - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_materials_consumption_by_project para usuario: {current_user}")
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # CONSULTA CORREGIDA - usando 'nombre' en lugar de 'nombre_proyecto'
        cursor.execute("""
            SELECT 
                p.id_proyecto,
                ...
                AVG(me.costo_unitario) as costo_unitario_promedio,
            ...
            GROUP BY p.id_proyecto, p.nombre, m.id, m.nombre
            ORDER BY p.nombre, cantidad_estimada DESC
        """)
        ...
        for r in rows:
            ...
            material = {
                "materialId": r[2],
                "name": r[3],
                "estimatedQuantity": float(r[4]) if r[4] else 0,
        ...
        print(f"✅ Consumo de {len(projects)} proyectos consultado por {current_user}")
...
# ---------------------------
# TOP MATERIALES MÁS USADOS - SOLO ADMIN
# ---------------------------
...
def get_top_used_materials(current_user):
    """
    Retorna los materiales más utilizados en proyectos - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_top_used_materials para usuario: {current_user}")
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # CONSULTA CORREGIDA PARA SQL SERVER - usando TOP en lugar de LIMIT
        cursor.execute("""
            SELECT TOP 10
                m.id,
                m.nombre,
            ...
            HAVING COALESCE(SUM(me.cantidad), 0) > 0
        ...
        top_materials = []
        for r in rows:
            total_used = float(r[3]) if r[3] else 0
            unit_price = float(r[5]) if r[5] else 0
            total_value = total_used * unit_price
        ...
        print(f"✅ Top {len(top_materials)} materiales más usados consultado por {current_user}")
...
# ---------------------------
# VALORIZACIÓN DE INVENTARIO - SOLO ADMIN
# ---------------------------
...
def get_inventory_valuation(current_user):
    """
    Retorna valorización del inventario por material - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_inventory_valuation para usuario: {current_user}")
        ...
        print(f"✅ Valorización de {len(valuation)} materiales consultada por {current_user}")
...
# ---------------------------
# DETALLE DE MATERIAL ESPECÍFICO - SOLO ADMIN
# ---------------------------
...
def get_material_detail(current_user, material_id):
    """
    Retorna detalle completo de un material (stock, uso, proyectos) - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_material_detail para usuario: {current_user}, material: {material_id}")
        ...
        # Proyectos donde se usa - CORREGIDO usando 'nombre' en lugar de 'nombre_proyecto'
        cursor.execute("""
            SELECT DISTINCT
                p.id_proyecto,
                p.nombre as nombre_proyecto,
            ...
            GROUP BY p.id_proyecto, p.nombre, e.nombre_etapa
        ...
        for r in cursor.fetchall():
            projects.append({
                ...
                "projectName": r[1],
                "stageName": r[2],
                "quantityUsed": float(r[3]) if r[3] else 0,
        ...
        print(f"✅ Detalle de material {material_id} consultado por {current_user}")
...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS CONSUMO ESTIMADO - SOLO ADMIN
# ---------------------------
...
def get_stock_comparison(current_user):
    """
    Compara stock actual vs consumo estimado en TODOS los proyectos - SOLO ADMINISTRADORES
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    
    try:
        print(f"📊 DEBUG - Ejecutando get_stock_comparison para usuario: {current_user}")
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # CONSULTA SIMPLE - Todos los proyectos sin filtrar por estado
        cursor.execute("""
            SELECT 
                m.id,
                m.nombre,
                m.cantidad as stock_actual,
                COALESCE((
                    SELECT SUM(me2.cantidad) 
                    FROM materiales_etapa me2
                    WHERE me2.id_material = m.id
                ), 0) as consumo_estimado,
                m.cantidad - COALESCE((
                    SELECT SUM(me2.cantidad) 
                    FROM materiales_etapa me2
                    WHERE me2.id_material = m.id
            ...
            WHERE EXISTS (
                SELECT 1 
                FROM materiales_etapa me3
                WHERE me3.id_material = m.id
            )
        ...
        for r in rows:
            current_stock = r[2]
            estimated_consumption = float(r[3]) if r[3] else 0
            difference = current_stock - estimated_consumption
            
            status = "sufficient" if difference >= 0 else "insufficient"
            ...
            comparison.append({
                ...
                "status": status,
                "needsRestock": difference < 0
            })
        
        print(f"✅ Comparación de {len(comparison)} materiales consultada por {current_user}")
