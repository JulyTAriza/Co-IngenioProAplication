...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (CORREGIDO)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        # 2️⃣ VALIDAR QUE EL PERSONAL_PROYECTO EXISTA (si se proporciona)
        id_personal = data.get("id_personal")
        if id_personal:
            cursor.execute("""
                SELECT id_personal_proyecto 
                FROM personal_proyecto 
                WHERE id_personal_proyecto = ? AND id_proyecto = ?
            """, (id_personal, id_proyecto))
            personal_existe = cursor.fetchone()
            
            if not personal_existe:
                conn.close()
                return jsonify(
                    success=False, 
                    message=f"El personal con ID {id_personal} no existe o no está asignado a este proyecto"
                ), 400
        ...
        """, (
            ...
            data["fecha_fin"],
            data["estado"],
            id_personal  # Puede ser NULL si no se proporciona
        ))
...
# ---------------------------
# ACTUALIZAR ACTIVIDAD (CORREGIDO)
# ---------------------------
...
def actualizar_actividad(current_user, id_actividad):
    ...
    try:
        ...
        # 1️⃣ OBTENER EL PROYECTO DE LA ACTIVIDAD PARA VALIDAR
