...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (CONVERSIÓN ID_USUARIO → ID_PERSONAL_PROYECTO)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        # 2️⃣ CONVERTIR ID_USUARIO EN ID_PERSONAL_PROYECTO PARA ESTE PROYECTO
        id_usuario_asignado = data.get("id_personal")  # En realidad es el id_usuario
        id_personal_final = None
        
        print(f"🔍 [CREAR-ACTIVIDAD] ID Usuario recibido: {id_usuario_asignado}")
        
        if id_usuario_asignado:
            # BUSCAR EL ID_PERSONAL_PROYECTO DE ESTE USUARIO EN ESTE PROYECTO
            cursor.execute("""
            ...
            if not personal_info:
                ...
                return jsonify(
                    success=False, 
                    message=f"El usuario con ID {id_usuario_asignado} no está asignado al proyecto {id_proyecto_etapa}"
                ), 400
            
            id_personal_final = personal_info[0]
            print(f"✅ [CREAR-ACTIVIDAD] Conversión: Usuario {id_usuario_asignado} → Personal {id_personal_final} en proyecto {id_proyecto_etapa}")
