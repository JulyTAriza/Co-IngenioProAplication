    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                u.username,
                u.e_mail,
                ISNULL(c.idioma, 'es') AS idioma,
                ISNULL(c.avatar, 'default_avatar.png') AS avatar
            FROM usuarios u
...
def update_user_settings(current_user):
    ...
    if not id_usuario:
        return jsonify(success=False, message="id_usuario es obligatorio"), 400
    ...
    try:
        ...
        if exists:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?)
            """, (id_usuario, idioma or "es", avatar or "default_avatar.png"))
