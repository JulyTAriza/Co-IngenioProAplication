...
def crear_proyecto(current_user):
    try:
        # DEBUG mejorado
        print("🔍 [CREATE PROYECTO] Headers:", dict(request.headers))
        raw_data = request.get_data(as_text=True)
        print("🔍 [CREATE PROYECTO] Raw data recibido:", raw_data)
        
        data = request.get_json(force=True, silent=True)
        if data is None:
            print("❌ [CREATE PROYECTO] No se pudo parsear JSON")
            return jsonify(success=False, message="JSON inválido - no se pudo parsear"), 400
            
        print("✅ [CREATE PROYECTO] JSON parseado correctamente:", data)
        
    except Exception as e:
        print("❌ [CREATE PROYECTO] Error parseando JSON:", str(e))
    ...
    try:
        ...
        descripcion = data.get("descripcion", "").strip() or ""
        departamento = data.get("departamento", "").strip() or ""
        email_cliente = data.get("email_cliente", "").strip() or ""
        telefono_cliente = data.get("telefono_cliente", "").strip() or ""
        direccion_cliente = data.get("direccion_cliente", "").strip() or ""
        ...
        print("🔄 [CREATE PROYECTO] Ejecutando SP con parámetros:")
        print(f"  nombre: {data['nombre'].strip()}")
        print(f"  descripcion: {descripcion}")
        print(f"  nombre_ciudad: {data['nombre_ciudad'].strip()}")
        print(f"  departamento: {departamento}")
        print(f"  nombre_cliente: {data['nombre_cliente'].strip()}")
        print(f"  email_cliente: {email_cliente}")
        print(f"  telefono_cliente: {telefono_cliente}")
        print(f"  direccion_cliente: {direccion_cliente}")
        ...
        if result and "id_proyecto" in columns:
            print(f"✅ [CREATE PROYECTO] Proyecto creado exitosamente: ID {result[2]}")
            ...
            print(f"❌ [CREATE PROYECTO] Error del SP: {result[1]}")
            return jsonify(success=False, message=result[1]), 400
        else:
            print("❌ [CREATE PROYECTO] No se pudo crear el proyecto - resultado inesperado")
            return jsonify(success=False, message="No se pudo crear el proyecto"), 400
    except Exception as e:
        print(f"❌ [CREATE PROYECTO] Error en base de datos: {str(e)}")
...
# -------------------------
# ELIMINAR PROYECTO (Soft Delete - cambia estado a "Eliminado")
# -------------------------
...
def eliminar_proyecto(current_user, proyecto_id):
    try:
        ...
        conn.commit()
        conn.close()
        return jsonify(success=True, message="Proyecto marcado como eliminado"), 200
    except Exception:
...
# -------------------------
# LISTAR PROYECTOS (excluye eliminados por defecto)
# -------------------------
...
def listar_proyectos(current_user):
    ...
    try:
        ...
        # Si no se especifica incluir_eliminados, excluirlos por defecto
        if not filtros["incluir_eliminados"]:
            if filtros["estado"]:
                # Si hay filtro de estado, usar ese
                estado_filtro = filtros["estado"]
            else:
                # Si no hay filtro, excluir "Eliminado"
                estado_filtro = "NOT Eliminado"
        else:
            estado_filtro = filtros["estado"]
        ...
        proyectos = [
            {
                ...
                "fecha_fin": r[4],
                "estado": r[5],
                "presupuesto": float(r[6]) if r[6] else None,
                "fecha_creacion": r[7],
                "ciudad": r[8],
                "cliente": r[9],
                "eliminado": r[5] == "Eliminado"  # Flag para frontend
            }
            for r in rows
        ]
...
def obtener_proyecto_detalle(current_user, proyecto_id):
    try:
        ...
        try:
            ...
            return jsonify(success=False, message="Error parseando datos del proyecto"), 500
...
# -------------------------
@projects_bp.route("/<int:proyecto_id>/restaurar", methods=["PUT"])
@token_required
def restaurar_proyecto(current_user, proyecto_id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Actualizar estado a "Planificación" para restaurar
        cursor.execute("""
            UPDATE proyectos 
            SET estado = 'Planificación' 
            WHERE id_proyecto = ? AND estado = 'Eliminado'
        """, (proyecto_id,))
        
        if cursor.rowcount == 0:
            conn.close()
            return jsonify(success=False, message="Proyecto no encontrado o no está eliminado"), 404
            
        conn.commit()
        conn.close()
        return jsonify(success=True, message="Proyecto restaurado exitosamente"), 200
        
    except Exception:
        current_app.logger.error("Error restaurando proyecto %s:\n%s", proyecto_id, traceback.format_exc())
        return jsonify(success=False, message="Error interno del servidor"), 500
