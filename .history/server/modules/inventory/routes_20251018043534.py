...
# ---------------------------
# CONSUMO DE MATERIALES POR PROYECTO (CORREGIDO)
# ---------------------------
...
# ---------------------------
# TOP MATERIALES MÁS USADOS (CORREGIDO)
# ---------------------------
...
def get_top_used_materials(current_user):
    ...
    try:
        ...
        top_materials = []
        for r in rows:
            total_value = float(r[3] or 0) * (float(r[5]) if r[5] else 0)
            top_materials.append({
                "id": r[0],
                "name": r[1],
                "currentStock": r[2],
                "totalUsed": float(r[3]) if r[3] else 0,
...
# ---------------------------
# DETALLE DE MATERIAL ESPECÍFICO (CORREGIDO)
# ---------------------------
...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS ESTIMADO (CORREGIDO)
# ---------------------------
    ...
    try:
        ...
        for r in rows:
            ...
            comparison.append({
                ...
                "name": r[1],
                "currentStock": r[2],
                "estimatedNeed": float(r[3]) if r[3] else 0,
                "difference": float(r[4]) if r[4] else 0,
                "status": status,
                "needsRestock": r[4] < 0
            })
...
# ---------------------------
# DEBUG - PARA IDENTIFICAR ERRORES
# ---------------------------
@inventory_bp.route("/debug/top-used", methods=["GET"])
@token_required
def debug_top_used(current_user):
    """
    Debug para identificar el error en top-used
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        print("🔍 DEBUG: Conectado a la base de datos")
        
        # Primero verifiquemos si hay datos en las tablas
        cursor.execute("SELECT COUNT(*) FROM Materiales")
        total_materials = cursor.fetchone()[0]
        print(f"🔍 DEBUG: Total materiales: {total_materials}")
        
        cursor.execute("SELECT COUNT(*) FROM materiales_etapa")
        total_materiales_etapa = cursor.fetchone()[0]
        print(f"🔍 DEBUG: Total materiales_etapa: {total_materiales_etapa}")
        
        cursor.execute("SELECT COUNT(*) FROM etapas_proyecto")
        total_etapas = cursor.fetchone()[0]
        print(f"🔍 DEBUG: Total etapas_proyecto: {total_etapas}")
        
        cursor.execute("SELECT COUNT(*) FROM proyectos")
        total_proyectos = cursor.fetchone()[0]
        print(f"🔍 DEBUG: Total proyectos: {total_proyectos}")
        
        # Ahora probemos la consulta paso a paso
        print("🔍 DEBUG: Probando consulta completa...")
        ...
        rows = cursor.fetchall()
        print(f"🔍 DEBUG: Consulta retornó {len(rows)} filas")
        
        for i, row in enumerate(rows):
            print(f"🔍 DEBUG: Fila {i}: {row}")
        ...
        return jsonify({
            ...
            "debug_info": {
                ...
                "total_materiales_etapa": total_materiales_etapa,
        ...
        print(f"❌ ERROR en debug_top_used: {str(e)}")
        print(traceback.format_exc())
        return jsonify({
            "success": False, 
            "error": str(e),
...
@inventory_bp.route("/debug/stock-comparison", methods=["GET"])
@token_required
def debug_stock_comparison(current_user):
    """
    Debug para identificar el error en stock-comparison
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        print("🔍 DEBUG STOCK: Conectado a la base de datos")
        
        # Verificar estados de proyectos
        cursor.execute("SELECT DISTINCT estado FROM proyectos")
        estados = cursor.fetchall()
        print(f"🔍 DEBUG STOCK: Estados de proyectos: {estados}")
        
        # Probar la consulta paso a paso
        print("🔍 DEBUG STOCK: Probando consulta completa...")
        ...
        rows = cursor.fetchall()
        print(f"🔍 DEBUG STOCK: Consulta retornó {len(rows)} filas")
        
        for i, row in enumerate(rows):
            print(f"🔍 DEBUG STOCK: Fila {i}: {row}")
        ...
        print(f"❌ ERROR en debug_stock_comparison: {str(e)}")
        print(traceback.format_exc())
        return jsonify({
            "success": False, 
            "error": str(e),
