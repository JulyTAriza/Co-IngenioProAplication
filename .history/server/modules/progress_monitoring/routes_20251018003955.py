...
def get_projects_progress(current_user):
    ...
    try:
        ...
        # Obtener todos los proyectos con sus tareas (columna correcta: 'nombre')
        cursor.execute("""
            SELECT 
                p.id_proyecto,
                p.nombre,
                ...
                SUM(CASE WHEN a.estado = 'Completada' THEN 1 ELSE 0 END) as completed_tasks
            FROM proyectos p
...
def get_users_progress(current_user):
    ...
    try:
        ...
        cursor.execute("""
            ...
            GROUP BY pp.id_personal_proyecto, pp.nombre, pp.rol
...
# ---------------------------
# DETALLE DE PROGRESO DE UN PROYECTO
# ---------------------------
...
def get_project_detail(current_user, project_id):
    ...
    try:
        ...
        # Información general del proyecto (columna correcta: 'nombre')
        cursor.execute("""
            SELECT 
                p.id_proyecto,
                p.nombre,
        ...
        project_info = {
            ...
            "startDate": project_row[3].strftime('%Y-%m-%d') if r[3] else None,
            "endDate": project_row[4].strftime('%Y-%m-%d') if r[4] else None,
            "totalTasks": total,
            "completedTasks": completed,
            "percentage": percentage
        }
...
def get_progress_summary(current_user):
    ...
    try:
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM actividades_cronograma WHERE estado != 'Completada'
        """)
