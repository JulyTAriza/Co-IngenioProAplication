...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (LÓGICA SIMPLE)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        id_proyecto = etapa[0]
        print(f"✅ [CREAR-ACTIVIDAD] Etapa pertenece al proyecto: {id_proyecto}")
        
        # 2️⃣ VALIDACIÓN SIMPLE DEL ID_PERSONAL
        id_personal = data.get("id_personal")
        print(f"🔍 [CREAR-ACTIVIDAD] ID Personal recibido: {id_personal}")
        
        if id_personal:
            # VERIFICAR SIMPLEMENTE QUE EL ID_PERSONAL_PROYECTO EXISTA Y TENGA USUARIO VÁLIDO
            cursor.execute("""
                SELECT pp.id_personal_proyecto, pp.id_usuario, u.username 
            ...
            print(f"✅ [CREAR-ACTIVIDAD] Personal válido: ID={personal_info[0]}, Usuario={personal_info[2]} (ID:{personal_info[1]})")
