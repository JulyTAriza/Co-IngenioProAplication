...
import { Plus, ChevronLeft, ChevronRight, Calendar, Edit2, Trash2, X, Save, User, FolderOpen, Folder, FileText, Users, Clock, Search, AlertTriangle, CheckCircle } from 'lucide-react';
import { 
...
type EstadoType = "Finalizada" | "En Progreso" | "Pendiente"

const ESTADO_COLORS: Record<EstadoType, { bg: string; light: string; ring: string }> = {
  "Finalizada": { bg: "bg-green-500", light: "bg-green-100 text-green-700", ring: "ring-green-500" },
...
function ModalActividad({ isOpen, onClose, onActividadCreada, idEtapa, actividadEdit, onShowSuccess, onShowError }: ModalActividadProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (actividadEdit) {
        ...
        onShowSuccess('Éxito', 'Actividad actualizada correctamente');
      } else {
        await createActividad({ 
          id_etapa: idEtapa, 
          data: formData 
        }).unwrap();
        onShowSuccess('Éxito', 'Actividad creada correctamente');
      }
      onClose();
      ...
      onShowError('Error', error?.data?.message || 'Error al guardar');
    }
  };
...
// Componente de carpeta visual para proyectos
function ProjectFolder({ proyecto, onClick }: { proyecto: any; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group relative bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 p-6 text-left border-2 border-transparent hover:border-blue-300 hover:scale-105"
    >
      {/* Icono de carpeta */}
      ...
      <div className="space-y-2 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          <Users size={14} />
          <span className="truncate">{proyecto.nombre_cliente || 'Cliente'}</span>
        </div>
      ...
      <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-transparent via-blue-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity -z-10" />
    </button>
  );
}
...
export default function CronogramaGantt() {
  ...
  const filteredProyectos = proyectos?.filter((proyecto) =>
    ...
    proyecto.nombre_cliente?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  // 🔥 MODIFICADO: Función para eliminar actividad con modal personalizado
  const handleDeleteActividad = (idActividad: number) => {
    setConfirmModal({
      isOpen: true,
  ...
  // 🔥 MODIFICADO: Función para toggle completar actividad
  const handleToggleComplete = async (actividad: Actividad) => {
    const nuevoEstado = actividad.estado === 'finalizada' ? 'En Progreso' : 'finalizada';
    try {
      await updateActividad({
      ...
      setErrorModal({
        isOpen: true,
        title: "Error",
        message: error?.data?.message || 'Error al actualizar'
      });
    }
  };

  // 🔥 NUEVO: Funciones para mostrar modales desde ModalActividad
  ...
  // Si no se ha seleccionado un proyecto - VISTA DE CARPETAS
  if (!selectedProyectoId) {
    return (
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
              ...
              {filteredProyectos.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">
                    {searchTerm ? "Intenta con otros términos de búsqueda" : "No hay proyectos creados"}
                  </p>
                </div>
              ) : (
            ...
            <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-blue-800">Diagrama de Gantt</h3>
                  <p className="text-blue-600 text-sm">
                    Visualiza y gestiona el cronograma de tus proyectos de manera intuitiva
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <button
                    onClick={() => setSelectedProyectoId(null)}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 active:bg-blue-800 transition-all duration-200 text-sm font-medium shadow-sm hover:shadow-md"
                  >
                    Cambiar proyecto
                  </button>
                </div>
                ...
                {expandedEtapas.includes(etapa.id_etapa) && etapa.actividades?.map((actividad) => (
                  ...
                  >
                    <div className="px-4 py-3 pl-10 border-r flex items-center gap-3 sticky left-0 bg-white z-10">
                      <input 
                        type="checkbox" 
                        checked={actividad.estado === "finalizada"} 
      ...
      {selectedEtapa && (
        <ModalActividad
          ...
          onActividadCreada={handleRefetchData}
