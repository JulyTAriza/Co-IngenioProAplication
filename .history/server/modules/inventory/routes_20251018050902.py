...
# ---------------------------
# TOP MATERIALES MÁS USADOS (CORREGIDO PARA SQL SERVER)
# ---------------------------
...
# ---------------------------
# COMPARACIÓN STOCK ACTUAL VS CONSUMO ESTIMADO (CORREGIDA)
# ---------------------------
@inventory_bp.route("/stock-comparison", methods=["GET", "OPTIONS"])
@token_required
def get_stock_comparison(current_user):
    """
    Compara stock actual vs consumo estimado en proyectos NO ELIMINADOS
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'preflight ok'}), 200
    ...
    try:
        ...
        # CONSULTA CORREGIDA - Usar "En planificación" en lugar de "Activo"
        cursor.execute("""
            SELECT 
                m.id,
                m.nombre,
                ...
                COALESCE((
                    ...
                    INNER JOIN etapas_proyecto ep ON me2.id_etapa = ep.id_etapa
                    INNER JOIN proyectos p ON ep.id_proyecto = p.id_proyecto
                    WHERE me2.id_material = m.id AND p.estado != 'Eliminado'
            ...
            WHERE EXISTS (
                ...
                INNER JOIN etapas_proyecto ep ON me3.id_etapa = ep.id_etapa
                INNER JOIN proyectos p ON ep.id_proyecto = p.id_proyecto
                WHERE me3.id_material = m.id AND p.estado != 'Eliminado'
            )
        ...
        print(f"✅ Comparación de {len(comparison)} materiales con consumo estimado en proyectos NO ELIMINADOS")
