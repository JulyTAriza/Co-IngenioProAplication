...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (CORREGIDO - SIN COLUMNA NOMBRE)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        # 3️⃣ BUSCAR EL PERSONAL_PROYECTO DEL USUARIO EN ESTE PROYECTO ESPECÍFICO (SIN COLUMNA NOMBRE)
        cursor.execute("""
            SELECT id_personal_proyecto, rol
        ...
        for personal in personal_usuario:
            print(f"   - ID: {personal[0]}, Rol: {personal[1]}")
        ...
        if id_personal:
            ...
            if not personal_valido:
                ...
                # Buscar todos los personal disponibles en este proyecto (SIN COLUMNA NOMBRE)
                cursor.execute("""
                    SELECT id_personal_proyecto, rol 
                    FROM personal_proyecto 
                    WHERE id_proyecto = ?
