...
def get_projects_progress(current_user):
    ...
    try:
        ...
        print(f"✅ Progreso de {len(projects)} proyectos calculado (excluyendo eliminados)")
...
# ---------------------------
# PROGRESO POR USUARIOS (CORREGIDO CON FILTRO ELIMINADOS)
# ---------------------------
...
def get_users_progress(current_user):
    ...
    try:
        ...
        # Consulta corregida: usar 'finalizada' y filtrar proyectos eliminados
        cursor.execute("""
            SELECT 
        ...
        print(f"✅ Progreso de {len(users)} usuarios calculado (excluyendo proyectos eliminados)")
...
# ---------------------------
# DETALLE DE PROGRESO DE UN PROYECTO (CORREGIDO CON FILTRO ELIMINADOS)
# ---------------------------
...
def get_project_detail(current_user, project_id):
    ...
    try:
        ...
        print(f"✅ Detalle de proyecto {project_id} calculado (proyecto activo)")
...
# ---------------------------
# RESUMEN GENERAL (DASHBOARD) (CORREGIDO CON FILTRO ELIMINADOS)
# ---------------------------
...
def get_progress_summary(current_user):
    ...
    try:
        ...
        print(f"✅ Resumen general calculado (excluyendo proyectos eliminados)")
...
def check_and_update_project_status(project_id):
    ...
    try:
        ...
        if total_tasks > 0 and total_tasks == completed_tasks:
            ...
            conn.commit()
            print(f"✅ Proyecto {project_id} actualizado a estado 'finalizada' (todas las tareas completadas)")
