...
app.register_blueprint(login_bp)  # Login queda en /login
app.register_blueprint(materials_bp, url_prefix="/materials")
app.register_blueprint(users_bp, url_prefix="/usuarios")
app.register_blueprint(projects_bp, url_prefix="/projects")
...
# Habilitar método OPTIONS en rutas (para preflight)
for bp in [login_bp, materials_bp, users_bp, projects_bp, notifications_bp]:
    for rule in bp.url_map.iter_rules():
        if "OPTIONS" not in rule.methods:
