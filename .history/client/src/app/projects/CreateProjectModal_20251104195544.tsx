...
export default function CreateProjectModal({
  ...
  useEffect(() => {
    ...
    console.log("🔍 [FRONTEND-DEBUG] ========== MODAL ABIERTO ==========");
    console.log("📋 Proyecto recibido:", proyecto);
    console.log("📦 Materiales disponibles (materials):", materials.map(m => ({id: m.id, nombre: m.nombre})));

    if (proyecto) {
      console.log("🎯 Cargando proyecto para editar:", proyecto.id_proyecto);
      console.log("📦 Materiales del proyecto recibidos:", proyecto.materiales);
      console.log("🔢 Número de materiales:", proyecto.materiales?.length || 0);
      console.log("🆔 IDs de materiales:", proyecto.materiales?.map((m: any) => m.id_material));
      ...
      const materialesValidados = Array.isArray(proyecto.materiales) 
        ? proyecto.materiales.filter((material: any) => 
            material && 
            typeof material.id_material === 'number' &&
            material.id_material > 0 &&
            typeof material.cantidad === 'number'
          )
        : [];

      console.log("✅ Materiales validados después de filtro:", materialesValidados);
      console.log("🆔 IDs de materiales validados:", materialesValidados.map((m: any) => m.id_material));
      ...
      console.log("🎯 Estado final de formData.materiales:", materialesValidados);
      console.log("🔍 [FRONTEND-DEBUG] ========== FIN CARGA ==========");
      
    } else {
      console.log("🆕 Creando nuevo proyecto - formulario reseteado");
      resetForm();
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log("🆔 IDs de materiales enviados:", payload.materiales.map((m: any) => m.id_material));
      ...
      setSuccessModal({
        isOpen: true,
        title: "¡Éxito!",
        message: proyecto ? "Proyecto actualizado correctamente" : "Proyecto creado correctamente"
      });
