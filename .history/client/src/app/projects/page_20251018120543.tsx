...
const ProjectsPage = () => {
  ...
  const handleEdit = async (proyecto: ProyectoResumen) => {
    try {
      ...
      // Resetear estados primero - INCLUYENDO retryCount
      setIsLoadingEdit(true);
      setSelectedProyecto(null);
      setLastRequestedId(proyecto.id_proyecto);
      setRetryCount(0); // ← SIEMPRE resetear aquí
    ...
    if (proyectoDetalleResponse && !loadingDetalle && proyectoIdParaDetalle) {
      ...
      if (respuestaId && respuestaId === solicitadoId) {
        ...
        if (retryCount < 2) {
          ...
          console.log(`🔄 [PAGE-DEBUG] Reintento ${newRetryCount}/2...`);
          ...
          alert("Error: No se pudo cargar el proyecto correctamente. Intente nuevamente.");
          setIsLoadingEdit(false);
          setProyectoIdParaDetalle(null);
          setLastRequestedId(null);
          setRetryCount(0);
  ...
  // Effect adicional para forzar recarga cuando cambia el ID solicitado - CORREGIDO
  useEffect(() => {
    if (proyectoIdParaDetalle && proyectoIdParaDetalle !== lastRequestedId) {
      ...
      setRetryCount(0); // ← CRÍTICO: Resetear contador cuando cambia el ID
      refetchDetalle();
    }
