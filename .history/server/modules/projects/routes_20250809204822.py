...
def crear_proyecto(current_user):
    ...
    required = ["nombre", "fecha_inicio", "estado", "equipo", "materiales"]
    missing = [k for k in required if not data.get(k)]
    if missing:
        return jsonify(success=False, message=f"Faltan campos obligatorios: {', '.join(missing)}"), 400

    # Validación: Debe venir id_ciudad o nombre_ciudad
    if not data.get("id_ciudad") and not data.get("nombre_ciudad"):
        return jsonify(success=False, message="Debe enviar id_ciudad o nombre_ciudad"), 400

    # Validación: Debe venir id_cliente o nombre_cliente
    if not data.get("id_cliente") and not data.get("nombre_cliente"):
        return jsonify(success=False, message="Debe enviar id_cliente o nombre_cliente"), 400
    ...
    try:
        ...
        """, (
            ...
            int(data["id_ciudad"]) if data.get("id_ciudad") else None,
            data.get("nombre_ciudad"),
