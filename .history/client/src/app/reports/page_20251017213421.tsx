...
const SimpleReportes = () => {
  ...
  const chartToImage = async (selector: string) => {
  ...
  const downloadPDF = async () => {
    const materialesChart = await chartToImage("#chart-materiales");
    const clientesChart = await chartToImage("#chart-clientes");
    const avancesChart = await chartToImage("#chart-avances");

    const docDefinition: any = {
      content: [
        { text: "Reporte General del Sistema", style: "header" },
        { text: `Fecha de generación: ${new Date().toLocaleDateString()}`, style: "subheader" },
        { text: "\n" },

        { text: "📦 Materiales", style: "sectionHeader" },
        materialesChart ? { image: materialesChart, width: 400, alignment: "center" } : { text: "No hay gráfico disponible." },
        { text: "\n" },

        { text: "👥 Clientes", style: "sectionHeader" },
        clientesChart ? { image: clientesChart, width: 400, alignment: "center" } : { text: "No hay gráfico disponible." },
        { text: "\n" },

        { text: "📈 Avances", style: "sectionHeader" },
        avancesChart ? { image: avancesChart, width: 400, alignment: "center" } : { text: "No hay gráfico disponible." },
      ],
      styles: {
        header: { fontSize: 22, bold: true, alignment: "center", color: "#1E3A8A" },
        subheader: { fontSize: 12, italics: true, alignment: "center", color: "#6B7280" },
        sectionHeader: { fontSize: 16, bold: true, color: "#111827", margin: [0, 10, 0, 4] },
      },
      defaultStyle: { font: "Helvetica" },
    };

    pdfMake.createPdf(docDefinition).download(`Reporte_${new Date().toISOString().split("T")[0]}.pdf`);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div id="chart-materiales" className="bg-white p-6 rounded-xl shadow-sm">
          ...
          <div id="chart-clientes" className="bg-white p-6 rounded-xl shadow-sm">
          ...
          <div id="chart-avances" className="bg-white p-6 rounded-xl shadow-sm">
