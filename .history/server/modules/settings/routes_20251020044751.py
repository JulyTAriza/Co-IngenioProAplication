...
def solicitar_codigo_contrasena(current_user, current_user_id, id_usuario):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO CodigosVerificacion (id_usuario, codigo, fecha_expiracion, intentos, utilizado, tipo)
            VALUES (?, ?, ?, 0, 0, 'cambio_contrasena')
...
def cambiar_contrasena(current_user, current_user_id, id_usuario):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE id_usuario = ? AND fecha_expiracion > GETDATE() AND utilizado = 0 AND tipo = 'cambio_contrasena'
