...
const ROLES_PERMITIDOS = ['Operario', 'Supervisor'];
...
const Users = () => {
  ...
  // Verificar si el usuario seleccionado es administrador (convertir a boolean explícitamente)
  const isSelectedUserAdmin = Boolean(
    selectedUserId && users?.find(u => u.id === selectedUserId)?.rol === 'Administrador'
  );

  const handleDelete = async () => {
    if (selectedUserId !== null && !isSelectedUserAdmin) {
      try {
  ...
  const getRoleChip = (rol: string) => {
    const color = rol === 'Administrador' ? 'error' : 
                  rol === 'Supervisor' ? 'warning' : 'primary';
    
    const icon = rol === 'Administrador' ? <Shield size={16} /> :
                 rol === 'Supervisor' ? <Edit size={16} /> :
                 <User size={16} />;
    ...
    <div className="flex flex-col px-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        <div className="flex items-center gap-2 text-sm text-blue-800">
          <Shield size={16} />
          <span>
            <strong>Nota:</strong> Los usuarios con rol <strong>Administrador</strong> no pueden ser modificados o eliminados.
          </span>
        </div>
      </div>
      ...
      <div className="flex justify-end mt-4 gap-2">
        ...
        <Button
          ...
          disabled={!selectedUserId || isSelectedUserAdmin}
