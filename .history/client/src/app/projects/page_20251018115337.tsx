...
const ProjectsPage = () => {
  ...
  const { 
    ...
    refetchOnMountOrArgChange: true, // Forzar recarga cada vez
  });
  ...
  useEffect(() => {
    console.log("🔍 [PAGE-DEBUG] useEffect detalles - Estado:", {
      ...
      responseId: proyectoDetalleResponse?.id_proyecto // ← CORREGIDO: quitado .data
    });

    // DEBUG: Verificar estructura de la respuesta
    if (proyectoDetalleResponse) {
      console.log("🔍 ESTRUCTURA DE RESPUESTA:", {
        respuestaCompleta: proyectoDetalleResponse,
        tipo: typeof proyectoDetalleResponse,
        keys: proyectoDetalleResponse ? Object.keys(proyectoDetalleResponse) : 'null'
      });
    }
    ...
    if (proyectoDetalleResponse && !loadingDetalle && proyectoIdParaDetalle) {
      ...
      const respuestaId = proyectoDetalleResponse.id_proyecto; // ← CORREGIDO: quitado .data
      ...
      if (respuestaId && respuestaId === solicitadoId) {
        ...
        setSelectedProyecto(proyectoDetalleResponse); // ← CORREGIDO: quitado .data
        setIsModalOpen(true);
        ...
        // Forzar recarga manual si hay discrepancia
        console.log("🔄 [PAGE-DEBUG] Forzando recarga manual...");
        refetchDetalle();
        return; // No continuar
      }
  ...
  }, [proyectoDetalleResponse, loadingDetalle, errorDetalle, proyectoIdParaDetalle, refetchDetalle]);
  ...
  return (
    <div className="mx-auto pb-5 w-full">
      ...
      <CreateProjectModal
        key={selectedProyecto?.id_proyecto || "new"} // ← ESTO ES CRÍTICO
        isOpen={isModalOpen}
        onClose={handleCloseModal}
        onCreate={handleCreateProyecto}
