...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    if (usuarioGuardado) {
      try {
        ...
        console.log("Usuario cargado:", usuarioData); // Para debug
      } catch (error) {
  ...
  console.log("Es operario:", isOperario); // Para debug
  ...
  // Filtrar links según el rol - MOSTRAR TODOS HASTA QUE SE CARGUE EL USUARIO
  const visibleLinks = usuario 
    ? (isOperario 
        ? allLinks.filter(link => link.allowedForOperario)
        : allLinks)
    : allLinks; // Mientras carga, muestra todos

  console.log("Links visibles:", visibleLinks.length); // Para debug
