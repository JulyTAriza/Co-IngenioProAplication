...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO actividades_cronograma (id_etapa, nombre_actividad, descripcion, fecha_inicio, fecha_fin, estado, id_personal_proyecto)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            id_etapa,
            data["nombre"],  # Se guarda como nombre_actividad
...
def actualizar_actividad(current_user, id_actividad):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE id_actividad = ?
        """, (
            data.get("nombre"),  # Se actualiza como nombre_actividad
            data.get("descripcion"),
