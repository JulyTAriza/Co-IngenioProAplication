...
const ReportesAvances: React.FC = () => {
  ...
  // getReporteEtapas expects a number (id_proyecto)
  const {
    data: etapas = [],
  ...
  // getReporteAvances expects a number (id_etapa)
  const {
    data: avances = [],
  ...
  // getReporteMateriales is void in api -> call undefined when we want it, skipToken otherwise
  const { 
    data: repMateriales,
    isLoading: loadingMateriales,
    isError: errorMateriales 
  } = useGetReporteMaterialesQuery(
    // Fetch only when user selected the report AND selected a project (you had validation)
    tipoReporteExtra === "materiales" && selectedProjectId !== ""
      ? undefined
      : skipToken
  );

  // getReporteMaterialesPorEtapa expects id_etapa (number)
  const { 
  ...
  // getReportePersonalProyecto expects id_proyecto (number)
  const { 
  ...
  // endpoints void: fetch only when corresponding tipoReporteExtra selected
  const { 
    data: repEstadosPersonal,
    isLoading: loadingEstadosPersonal,
    isError: errorEstadosPersonal 
  } = useGetReporteEstadosPersonalQuery(
    tipoReporteExtra === "estadosPersonal" ? undefined : skipToken
  );
  ...
  const { 
    ...
    tipoReporteExtra === "clientes" ? undefined : skipToken
  );
