...
def construir_detalle_manualmente(rows, columns):
    ...
    try:
        ...
        # Procesar equipo y materiales si están en las filas
        for row in rows:
            # Agregar lógica para equipo si existe en las columnas
            if "id_personal_proyecto" in columns and row[columns.index("id_personal_proyecto")]:
                equipo_member = {
                    "id_personal_proyecto": row[columns.index("id_personal_proyecto")],
                    "nombre": row[columns.index("personal_nombre")] if "personal_nombre" in columns else None,
                    "rol": row[columns.index("rol")] if "rol" in columns else None
                }
                if equipo_member not in detalle["equipo"]:
                    detalle["equipo"].append(equipo_member)
            
            # Agregar lógica para materiales si existe en las columnas
            if "id_material" in columns and row[columns.index("id_material")]:
                material = {
                    "id_material": row[columns.index("id_material")],
                    "nombre": row[columns.index("material_nombre")] if "material_nombre" in columns else None,
                    "cantidad": row[columns.index("cantidad")] if "cantidad" in columns else None,
                    "unidad": row[columns.index("unidad")] if "unidad" in columns else None
                }
                if material not in detalle["materiales"]:
                    detalle["materiales"].append(material)
        
        print(f"✅ [CONSTRUIR-MANUAL] Detalle construido: {len(detalle['equipo'])} equipo, {len(detalle['materiales'])} materiales")
        return detalle
