...
def complete_task(current_user, task_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT a.id_actividad, a.nombre_actividad, a.id_personal_proyecto, pp.id_usuario
            FROM actividades_cronograma a
        ...
        print(f"📝 [COMPLETE-TASK] Tarea encontrada: ID={task[0]}, Nombre='{task[1]}', Personal={task[2]}, UsuarioAsignado={task[3]}")
        
        # Verificar que la tarea está asignada a este usuario
        if task[3] != id_usuario:
            conn.close()
            print(f"🚫 [COMPLETE-TASK] Permiso denegado: Tarea usuario={task[3]}, Usuario actual={id_usuario}")
        ...
        print(f"✅ [COMPLETE-TASK] Tarea {task_id} completada por {current_user}")
