...
# ---------------------------
# CREAR NUEVA ACTIVIDAD EN ETAPA (CON DIAGNÓSTICO COMPLETO)
# ---------------------------
...
def crear_actividad(current_user, id_etapa):
    ...
    try:
        ...
        id_proyecto = etapa[0]
        print(f"✅ [CREAR-ACTIVIDAD] Proyecto de la etapa: {id_proyecto}")
        
        # 2️⃣ DIAGNÓSTICO: VER TODOS LOS PERSONAL_PROYECTO DEL PROYECTO
        cursor.execute("""
            SELECT id_personal_proyecto, nombre, rol, id_usuario
            FROM personal_proyecto 
            WHERE id_proyecto = ?
        """, (id_proyecto,))
        personal_disponible = cursor.fetchall()
        
        print(f"👥 [CREAR-ACTIVIDAD] Personal disponible en proyecto {id_proyecto}:")
        for personal in personal_disponible:
            print(f"   - ID: {personal[0]}, Nombre: {personal[1]}, Rol: {personal[2]}, Usuario: {personal[3]}")
        
        # 3️⃣ VALIDAR QUE EL PERSONAL_PROYECTO EXISTA
        id_personal = data.get("id_personal")
        print(f"🔍 [CREAR-ACTIVIDAD] ID Personal recibido del frontend: {id_personal}")
        
        if id_personal:
            # Verificar si el personal existe en ESTE proyecto
            personal_ids = [p[0] for p in personal_disponible]
            print(f"🔍 [CREAR-ACTIVIDAD] IDs válidos en este proyecto: {personal_ids}")
            
            if id_personal not in personal_ids:
                conn.close()
                return jsonify(
                    success=False, 
                    message=f"El personal con ID {id_personal} no está asignado a este proyecto. IDs válidos: {personal_ids}"
                ), 400
        
        # 4️⃣ SI NO HAY PERSONAL, USAR NULL
        if not id_personal:
            id_personal = None
            print("ℹ️  [CREAR-ACTIVIDAD] No se asignó personal, usando NULL")
        
        # 5️⃣ INSERTAR LA ACTIVIDAD
        print(f"🔄 [CREAR-ACTIVIDAD] Insertando actividad con personal_id: {id_personal}")
